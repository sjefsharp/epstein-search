...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "KeyFiguresPage" });
...
export default async function KeyFiguresPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "KeyFiguresPage" });
      ...
      <div className="mx-auto w-full max-w-4xl px-6 py-12">
        ...
        <article className="space-y-8 text-sm text-foreground">
          <section className="space-y-3">
            <h2 className="text-base font-semibold">{t("epsteinTitle")}</h2>
            <p className="text-muted-foreground leading-relaxed">{t("epstein")}</p>
          </section>

          <section className="space-y-3">
            <h2 className="text-base font-semibold">{t("maxwellTitle")}</h2>
            <p className="text-muted-foreground leading-relaxed">{t("maxwell")}</p>
          </section>

          <section className="space-y-3">
            <h2 className="text-base font-semibold">{t("victimsTitle")}</h2>
            <p className="text-muted-foreground leading-relaxed">{t("victims")}</p>
          </section>

          <section className="space-y-3">
            <h2 className="text-base font-semibold">{t("investigatorsTitle")}</h2>
            <p className="text-muted-foreground leading-relaxed">{t("investigators")}</p>
          </section>
